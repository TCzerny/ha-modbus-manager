...
async def async_setup_entry(
    ...
    _LOGGER.debug(
        "Select Setup: prefix=%s, template=%s, registers=%d, controls=%d",
        prefix,
        template_name,
        len(registers),
        len(controls),
    ...
    for reg in registers:
        if reg.get("control") == "select":
            ...
            if entity_id in existing_entities:
                _LOGGER.debug(
                    "Select Entity %s existiert bereits, überspringe", entity_id
                )
                continue

            entities.append(
                ModbusTemplateSelect(
                    hass=hass,
                    name=sensor_name,
                    unique_id=unique_id,
                    entry=entry,
    ...
    for control in controls:
        ...
        has_select_properties = (
            "map" in control or "options" in control or "flags" in control
        )
        ...
        if is_explicit_select or has_select_properties:
            ...
            if entity_id in existing_entities:
                _LOGGER.debug(
                    "Select Control Entity %s existiert bereits, überspringe", entity_id
                )
                continue
    ...
    async def async_update(self):
        ...
        try:
            ...
            if self._entry.entry_id not in self.hass.data[DOMAIN]:
                _LOGGER.error(
                    "Select %s: No configuration data found for Entry %s",
                    self.name,
            ...
            if not result or not hasattr(result, "registers"):
                _LOGGER.warning(
                    "Select %s: Fehler beim Lesen von Register %s: %s",
                    self.name,
                    self._address,
                    result,
                )
            ...
            _LOGGER.debug(
                "Select %s: Raw value aus Register: %s", self.name, processed_value
            )

            if processed_value is not None:
                # Apply value processing (same logic as sensors: map -> flags -> options)
                _LOGGER.debug(
                    "Select %s: Before value processing: map=%s, flags=%s, options=%s",
                    self.name,
                    bool(self._map),
                    bool(self._flags),
                    bool(self._options),
                ...
                if final_value is not None:
                    ...
                    _LOGGER.debug(
                        "Select %s: Wert %s -> Verarbeiteter Wert %s",
                        self.name,
                        processed_value,
                        final_value,
                    )
                    ...
                    _LOGGER.debug(
                        "Select %s: Wert %s -> Keine Verarbeitung, verwende %s",
                        self.name,
                        processed_value,
    ...
    async def async_select_option(self, option: str) -> None:
        ...
        try:
            ...
            if numeric_value is None:
                _LOGGER.error(
                    "Unbekannte Option: %s. Verfügbare Optionen: %s",
                    option,
            ...
            if self._input_type == "holding":
                ...
                if result.isError():
                    ...
                    _LOGGER.debug(
                        "Option %s erfolgreich in Register %s geschrieben",
                        option,
                        self._address,
                    )
        ...
        try:
            ...
            if self._value_to_text and isinstance(value, (int, float)):
                ...
                if int_value in self._value_to_text:
                    ...
                    _LOGGER.debug(
                        "Value %s not found in options for %s", int_value, self.name
                    )
