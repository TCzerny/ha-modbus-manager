...
class ModbusManagerConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_device_config(self, user_input: dict = None) -> FlowResult:
        ...
        if user_input is not None:
            ...
            _LOGGER.debug("All config values: phases=%s, mppt_count=%s, battery_config=%s", 
                         user_input.get("phases"), user_input.get("mppt_count"), battery_config)
            if battery_config == "sbr_battery":
                # Store current input and go to battery slave ID step
                _LOGGER.info("SBR Battery selected, redirecting to battery slave ID step")
        ...
        if template_data.get("is_simple_template"):
            ...
            if "SunSpec Standard Configuration" in self._selected_template:
                ...
                return self.async_show_form(
                    ...
                    description_placeholders={
                        "template": self._selected_template,
                        "description": template_data.get("description", "Simplified Template")
                    }
                )
        else:
            ...
            schema_fields = {
                ...
                vol.Optional("message_wait_milliseconds", default=DEFAULT_MESSAGE_WAIT_MS): int,
            }
    ...
    def _process_dynamic_config(self, user_input: dict, template_data: dict) -> dict:
        ...
        _LOGGER.debug("_process_dynamic_config user_input keys: %s", list(user_input.keys()))
        ...
        firmware_version = user_input.get("firmware_version", template_data.get("firmware_version", "1.0.0"))
        ...
        _LOGGER.debug("Dynamic config processing: battery_config=%s, battery_enabled=%s, battery_type=%s, battery_slave_id=%d", 
                     battery_config, battery_enabled, battery_type, battery_slave_id)
        _LOGGER.debug("Dynamic config parameters: phases=%d, mppt_count=%d, firmware_version=%s, connection_type=%s", 
                     phases, mppt_count, firmware_version, connection_type)
        ...
        for sensor in original_sensors:
            ...
            should_include = self._should_include_sensor(sensor, phases, mppt_count, battery_enabled, battery_type, battery_slave_id, firmware_version, connection_type, dynamic_config)
            ...
            if should_include:
                ...
                if "mppt3" in unique_id.lower() or "mppt 3" in sensor_name.lower():
                    _LOGGER.warning("MPPT3 sensor included despite mppt_count=%d: %s (unique_id: %s)", 
                                   mppt_count, sensor_name, unique_id)
            else:
                if "mppt3" in unique_id.lower() or "mppt 3" in sensor_name.lower():
                    _LOGGER.info("MPPT3 sensor correctly excluded: %s (unique_id: %s)", sensor_name, unique_id)
    ...
    def _should_include_sensor(self, sensor: dict, phases: int, mppt_count: int, battery_enabled: bool, 
        ...
        if "mppt" in search_text:
            ...
            _LOGGER.debug("MPPT filtering: search_text='%s', mppt_number=%s, mppt_count=%d", 
                         search_text, mppt_number, mppt_count)
            if mppt_number and mppt_number > mppt_count:
                ...
                _LOGGER.debug("Including MPPT sensor: %s (MPPT %d <= %d)", 
        ...
        if battery_type == "none":
            ...
            if sensor_group == "SBR_battery":
                _LOGGER.debug("Including SBR battery sensor: %s (unique_id: %s, group: %s)", 
                             sensor.get("name", "unknown"), sensor.get("unique_id", "unknown"), sensor_group)
            pass
    ...
    def _create_sunspec_config_entry(self, user_input: dict, template_data: dict, template_version: int) -> FlowResult:
        ...
        try:
            _LOGGER.debug("Creating SunSpec Standard Configuration Entry: %s", self._selected_template)
...
class ModbusManagerOptionsFlow(config_entries.OptionsFlow):
    ...
    async def async_step_init(self, user_input: dict = None) -> FlowResult:
        ...
        if user_input is not None:
            ...
            elif "battery_config" in user_input and user_input["battery_config"] == "sbr_battery":
        ...
        if template_last_updated > 0:
            import datetime
            last_updated_str = datetime.datetime.fromtimestamp(template_last_updated).strftime("%Y-%m-%d %H:%M:%S")
        else:
            last_updated_str = "Never"
        
        # Entity counts for display
        current_sensors_count = len(self.config_entry.data.get("registers", []))
        current_calculated_count = len(self.config_entry.data.get("calculated_entities", []))
        ...
        if template_data and isinstance(template_data, dict) and template_data.get("dynamic_config"):
            dynamic_config = template_data.get("dynamic_config", {})
            ...
            if "phases" in dynamic_config:
                ...
                phase_options = dynamic_config["phases"].get("options", [1, 3])
                _LOGGER.debug("Adding phases field: current=%s, options=%s", current_phases, phase_options)
                schema_fields[vol.Optional("phases", default=current_phases)] = vol.In(phase_options)
            
            # Add MPPT count selection
            if "mppt_count" in dynamic_config:
                current_mppt = self.config_entry.data.get("mppt_count", 2)
                mppt_options = dynamic_config["mppt_count"].get("options", [1, 2, 3])
                _LOGGER.debug("Adding mppt_count field: current=%s, options=%s", current_mppt, mppt_options)
                schema_fields[vol.Optional("mppt_count", default=current_mppt)] = vol.In(mppt_options)
            
            # Add battery configuration
            if "battery_config" in dynamic_config:
                current_battery = self.config_entry.data.get("battery_config", "none")
                battery_options = dynamic_config["battery_config"].get("options", ["none", "standard_battery", "sbr_battery"])
                option_labels = dynamic_config["battery_config"].get("option_labels", {})
                ...
                _LOGGER.debug("Adding battery_config field: current=%s, choices=%s", current_battery, battery_choices)
                schema_fields[vol.Optional("battery_config", default=current_battery)] = vol.In(battery_choices)
            
            # Add connection type
            if "connection_type" in dynamic_config:
                current_connection = self.config_entry.data.get("connection_type", "LAN")
                connection_options = dynamic_config["connection_type"].get("options", ["LAN", "WINET"])
                schema_fields[vol.Optional("connection_type", default=current_connection)] = vol.In(connection_options)
        
        # Add firmware version selection if available
        if available_firmware:
            current_firmware = self.config_entry.data.get("firmware_version", "1.0.0")
            schema_fields[vol.Optional("firmware_version", default=current_firmware)] = vol.In(available_firmware)
    ...
    async def async_step_aggregates_options(self, user_input: dict = None) -> FlowResult:
        ...
        try:
            ...
            if user_input is not None:
                ...
                _LOGGER.debug("Aggregate options updated: %d of %d aggregates selected", 
    ...
    async def async_step_update_template(self, user_input: dict = None) -> FlowResult:
        ...
        try:
            ...
            if isinstance(template_data, str):
                _LOGGER.error("Template data is string, expected dict: %s", template_data[:100])
                return self.async_abort(
            ...
            if dynamic_config:
                try:
                    ...
                    processed_data = self._process_dynamic_config({
                        ...
                        "battery_config": self.config_entry.data.get("battery_config", "none"),
                        "battery_slave_id": self.config_entry.data.get("battery_slave_id", 200),
                    ...
                    _LOGGER.info("Template processing completed: %d sensors, %d calculated, %d controls, %d binary_sensors", 
                                len(template_registers), len(calculated_entities), len(template_controls), len(template_binary_sensors))
                    
                except Exception as e:
                    _LOGGER.warning("Error applying dynamic config during template update, using original: %s", str(e))
            ...
            if user_input is not None:
                ...
                if template_registers:
                    ...
                    _LOGGER.info("Updated registers: %d sensors", len(template_registers))
                else:
                    _LOGGER.warning("No template_registers found, keeping existing registers")
                    ...
                    if isinstance(template_data, dict) and template_data.get("sensors"):
                        ...
                        _LOGGER.info("Used fallback: loaded %d sensors directly from template", len(template_data["sensors"]))
                ...
                _LOGGER.info("Template %s updated: v%s → v%s (%d sensors, %d calculated, %d controls, %d binary_sensors)", 
            ...
            stored_calculated_count = len(self.config_entry.data.get("calculated_entities", []))
            stored_controls_count = len(self.config_entry.data.get("controls", []))
            
            version_changed = current_version != stored_version
            content_changed = (current_sensors_count != stored_sensors_count or 
                             current_calculated_count != stored_calculated_count or
                             current_controls_count != stored_controls_count)
    ...
    async def async_step_battery_config(self, user_input: dict = None) -> FlowResult:
        ...
        return self.async_show_form(
            ...
            data_schema=vol.Schema({
                vol.Optional("battery_slave_id", default=current_battery_slave_id): int,
            }),
            description_placeholders={
                "battery_config": "SBR Battery"
            }
    ...
    async def async_step_apply_config_changes(self, user_input: dict) -> FlowResult:
        ...
        try:
            ...
            dynamic_params = ["phases", "mppt_count", "battery_config", "battery_slave_id", "connection_type"]
