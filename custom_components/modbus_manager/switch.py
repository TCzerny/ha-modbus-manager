...
class ModbusTemplateSwitch(SwitchEntity):
    ...
    async def async_update(self):
        ...
        try:
            ...
            result = await hub.read_holding_registers(
                self._address, self._count, unit=self._slave_id
            )

            if result.isError():
                _LOGGER.warning(
                    "Fehler beim Lesen von Holding Register %s: %s",
                    self._address,
                    result,
                )
                self._attr_is_on = False
    ...
    async def async_turn_on(self, **kwargs) -> None:
        ...
        try:
            ...
            _LOGGER.error(
                "Fehler beim Einschalten von Switch %s: %s", self.name, str(e)
            )
    ...
    async def async_turn_off(self, **kwargs) -> None:
        ...
        try:
            ...
            _LOGGER.error(
                "Fehler beim Ausschalten von Switch %s: %s", self.name, str(e)
            )
