...
class ModbusAggregateSensor(SensorEntity):
    ...
    def __init__(
        ...
        if aggregate_config:
            ...
            if "unique_id" in aggregate_config:
                ...
                _LOGGER.debug(
                    "Aggregate Sensor %s: Generated unique_id: %s",
                    self._name,
                    self._attr_unique_id,
                )
            ...
            device_name = (
                f"{self._prefix} Modbus Manager Aggregates"
                if self._prefix
                else "MM Modbus Manager Aggregates"
            )
            device_info = {
                "identifiers": {
                    (DOMAIN, f"modbus_manager_aggregation_hub_{self._prefix}")
                    if self._prefix
                    else (DOMAIN, "modbus_manager_aggregation_hub")
                },
                "name": device_name,
        ...
        try:
            ...
            if self._tracked_entities:
                ...
                self._unsubscribe = async_track_state_change_event(
                    self.hass, self._tracked_entities, self._state_changed
                )

                _LOGGER.debug(
                    "Tracking für %d Entitäten in Gruppe %s eingerichtet (sofort)",
                ...
                _LOGGER.debug(
                    "Keine Entitäten für Gruppe %s gefunden, globaler Listener eingerichtet",
                    self._group_tag,
                )

        except Exception as e:
            _LOGGER.error(
                "Fehler beim Einrichten des Trackings für Gruppe %s: %s",
                self._group_tag,
                str(e),
            )
    ...
    def _setup_global_listener(self):
        ...
        try:
            ...
            self._unsubscribe = async_track_state_change_event(
                self.hass, "sensor", self._global_state_changed  # Listen to all sensors
            )
            _LOGGER.debug(
                "Globaler Listener für Gruppe %s eingerichtet", self._group_tag
            )
        except Exception as e:
    ...
    def _find_group_entities(self):
        ...
        try:
            ...
            _LOGGER.debug(
                "Suche nach Entitäten für Gruppe %s mit Präfixen: %s",
                self._group_tag,
                modbus_prefixes,
            )
            ...
            for state in all_states:
                if state.domain == "sensor":
                    ...
                    if group_attr:
                        sensors_with_groups.append(
                            f"{state.entity_id} (group: {group_attr})"
                        )
            ...
            for state in all_states:
                if state.domain == "sensor":
                    ...
                    if group_attr == self._group_tag:
                        ...
                        is_aggregate_entity = (
                            "mm_" in state.entity_id.lower()
                            or "_aggregate_" in state.entity_id.lower()
                            or any(
                                f"{prefix}_aggregate_" in state.entity_id
                        ...
                        if is_aggregate_entity:
                            _LOGGER.debug(
                                "Überspringe %s - ist Aggregate-Sensor", state.entity_id
                            )
                            continue
                        ...
                        is_modbus_entity = any(
                            prefix.lower() in state.entity_id.lower()
                            or f"sensor.{prefix.lower()}_" in state.entity_id.lower()
                            for prefix in modbus_prefixes
                        ) or (
                            # Include calculated sensors (they have group attribute but no prefix)
                            state.entity_id.startswith("sensor.")
                            and group_attr is not None
                        )

                        _LOGGER.debug(
                            "Prüfe %s: is_modbus_entity=%s, prefixes=%s",
                            state.entity_id,
                            is_modbus_entity,
                            modbus_prefixes,
                        )
                        ...
                        if is_modbus_entity:
                            ...
                            is_aggregate_entity = (
                                ...
                                any(
                                    method in state.entity_id.lower()
                                    for method in [
                                        "_sum",
                                        "_max",
                                        "_min",
                            ...
                            if is_aggregate_entity:
                                _LOGGER.debug(
                                    "Überspringe %s - ist Aggregate-Sensor",
                                    state.entity_id,
                                )
                                continue
                            ...
                            _LOGGER.debug(
                                "Überspringe %s - kein Modbus Manager Entity",
                                state.entity_id,
                            )
            ...
            if not self._tracked_entities:
                _LOGGER.debug(
                    "Keine Entitäten mit Gruppe '%s' gefunden, versuche erweiterte Suche",
                    self._group_tag,
                )
                ...
                for state in all_states:
                    if state.domain == "sensor":
                        ...
                        if group_attr == self._group_tag:
                            ...
                            if is_aggregate_entity:
                                _LOGGER.debug(
                                    "Erweiterte Suche: Überspringe %s - ist Aggregate-Sensor",
                                    state.entity_id,
                                )
                                continue
    ...
    def _state_changed(self, event):
        ...
        try:
            ...
            _LOGGER.error(
                "Fehler bei State-Change für Gruppe %s: %s", self._group_tag, str(e)
            )
    ...
    def _global_state_changed(self, event):
        ...
        try:
            ...
            if group_attr:
                _LOGGER.debug(
                    "State-Change: %s (group: %s, state: %s)",
                    entity_id,
                    group_attr,
                    new_state.state,
                )
            ...
            if group_attr == self._group_tag:
                ...
                modbus_prefixes = self._get_modbus_prefixes()
                is_aggregate_entity = (
                    "mm_" in entity_id.lower()
                    or "_aggregate_" in entity_id.lower()
                    or any(
                        f"{prefix}_aggregate_" in entity_id
                        for prefix in modbus_prefixes
                    )
                    or
                    # Also exclude sensors with aggregation methods in their names
                    any(
                        method in entity_id.lower()
                        for method in ["_sum", "_max", "_min", "_average", "_count"]
                    )
                )

                if is_aggregate_entity:
                    _LOGGER.debug(
                        "Global State-Change: Überspringe %s - ist Aggregate-Sensor",
                        entity_id,
                    )
                    return
                ...
                # Check if it's a Modbus Manager entity
                is_modbus_entity = any(
                    prefix.lower() in entity_id.lower()
                    or f"sensor.{prefix.lower()}_" in entity_id.lower()
                    for prefix in modbus_prefixes
                ) or (entity_id.startswith("sensor.") and group_attr is not None)
                ...
                if is_modbus_entity and entity_id not in self._tracked_entities:
                    ...
                    _LOGGER.debug(
                        "Neue Entität für Gruppe %s gefunden: %s",
                        self._group_tag,
                        entity_id,
                    )
                    ...
                    _LOGGER.debug(
                        "Tracking für %d Entitäten in Gruppe %s aktualisiert",
                        len(self._tracked_entities),
                        self._group_tag,
                    )
            ...
            _LOGGER.error(
                "Fehler bei globalem State-Change für Gruppe %s: %s",
                self._group_tag,
                str(e),
            )
        ...
        try:
            ...
            for entity_id in self._tracked_entities:
                ...
                try:
                    ...
                    _LOGGER.debug(
                        "Entität %s hat keinen gültigen numerischen Wert: %s",
                        entity_id,
                        state.state,
                    )
                    continue
            ...
            _LOGGER.debug(
                "Aggregat %s aktualisiert: %s = %s (aus %d Entitäten: %s)",
                self._attr_name,
                self._method,
                result,
                valid_entities,
            ...
            _LOGGER.error(
                "Fehler bei der Aggregat-Berechnung für Gruppe %s: %s",
                self._group_tag,
                str(e),
            )
            self._attr_native_value = None
    ...
    async def _delayed_entity_search(self):
        ...
        try:
            ...
            if new_count > old_count:
                _LOGGER.debug(
                    "Verzögerte Suche: %d neue Entitäten für Gruppe %s gefunden",
                    new_count - old_count,
                    self._group_tag,
                )
                ...
                _LOGGER.debug(
                    "Verzögerte Suche: Keine neuen Entitäten für Gruppe %s gefunden",
                    self._group_tag,
                )

        except Exception as e:
            _LOGGER.error(
                "Fehler bei verzögerter Entity-Suche für Gruppe %s: %s",
                self._group_tag,
                str(e),
            )
...
class AggregationManager:
                ...
                if state.domain == "sensor":
                    ...
                    if group:
                        ...
                        if state.entity_id.startswith("sensor.") and any(
                            prefix in state.entity_id
                            for prefix in self._get_modbus_prefixes()
                        ):
                            groups.add(group)

            discovered_groups = list(groups)
            _LOGGER.debug(
                "Entdeckte Gruppen über alle Modbus Manager Geräte: %s",
                discovered_groups,
            )
    ...
    async def create_aggregate_sensors(
        self, group_tag: str, methods: List[str] = None
    ) -> List[ModbusAggregateSensor]:
        ...
        try:
            ...
            _LOGGER.debug(
                "Aggregat-Sensoren für Gruppe %s erstellt: %s",
                group_tag,
            ...
            _LOGGER.error(
                "Fehler beim Erstellen der Aggregat-Sensoren für Gruppe %s: %s",
                group_tag,
                str(e),
            )
            return []
