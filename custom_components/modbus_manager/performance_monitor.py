...
class PerformanceMonitor:
    ...
    def start_operation(self, device_id: str, operation_type: str, 
        ...
        try:
            ...
            operation_id = f"{device_id}_{operation_type}_{int(operation.start_time * 1000)}"
    ...
    def end_operation(self, device_id: str, operation_id: str, 
        ...
        try:
            summary = {
                "global": {
                    ...
                    "average_throughput": round(self.global_metrics.average_throughput, 2),
                    "last_operation": self.global_metrics.last_operation.isoformat() if self.global_metrics.last_operation else None
                },
                "devices": {}
            }
            ...
            for device_id, device_metrics in self.devices.items():
                summary["devices"][device_id] = {
                    ...
                    "last_operation": device_metrics.last_operation.isoformat() if device_metrics.last_operation else None
                }
            
            return summary
            
        except Exception as e:
            _LOGGER.error("Fehler beim Erstellen der Performance-Zusammenfassung: %s", str(e))
            return {}
    
    def get_recent_operations(self, device_id: str = None, 
    ...
    def _limit_global_history(self) -> None:
        ...
        if len(self.global_metrics.operations) > self.max_history:
            # Älteste Operationen entfernen
            self.global_metrics.operations = self.global_metrics.operations[-self.max_history:]
