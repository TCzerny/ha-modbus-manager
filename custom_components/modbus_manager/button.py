    ...
    for reg in registers:
        ...
        if reg.get("control") == "button":
            ...
            if entity_id in existing_entities:
                _LOGGER.debug(
                    "Button Entity %s existiert bereits, überspringe", entity_id
                )
                continue

            entities.append(
                ModbusTemplateButton(
                    hass=hass,
                    name=sensor_name,
                    unique_id=unique_id,
                    hub_name=hub_name,
...
class ModbusTemplateButton(ButtonEntity):
    ...
    async def _reset_after_duration(self):
        ...
        try:
            ...
            if result.isError():
                _LOGGER.error(
                    "Fehler beim Zurücksetzen von Button %s: %s", self.name, result
                )
            else:
            ...
            _LOGGER.error(
                "Fehler beim Zurücksetzen von Button %s: %s", self.name, str(e)
            )
