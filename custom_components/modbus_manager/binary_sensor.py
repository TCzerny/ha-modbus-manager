...
async def async_setup_entry(
    ...
    for binary_sensor in binary_sensors:
        _LOGGER.debug(
            "Processing binary sensor: %s (type: %s)",
            binary_sensor.get("name"),
            binary_sensor.get("type"),
        )
    ...
    if entities:
        ...
        _LOGGER.debug(
            "Created binary sensor entities: %s", [e.entity_id for e in entities]
        )
        ...
        try:
            ...
            if self._availability_template:
                try:
                    ...
                    _LOGGER.warning(
                        "Template error in availability for %s: %s", self.name, e
                    )
                    self._attr_is_on = False
                    return
            ...
            if self._state_template:
                try:
                    ...
                    if "int got invalid input 'unknown'" in str(e):
                        _LOGGER.debug(
                            "Binary sensor %s has 'unknown' value, setting to False",
                            self.name,
                        )
                        self._attr_is_on = False
                        ...
                        _LOGGER.warning(
                            "Template error in state for %s: %s", self.name, e
                        )
                        self._attr_is_on = False
            else:
                self._attr_is_on = False
            ...
            _LOGGER.error(
                "Fehler beim Update von Binary Sensor %s: %s", self.name, str(e)
            )
            self._attr_is_on = False
