...
def is_unavailable_register_value(value: Any, data_type: str, raw_registers: Optional[List[int]] = None) -> bool:
    ...
    try:
        ...
        if isinstance(value, (int, float)):
            ...
            if int_value == error_value:
                _LOGGER.debug("Detected unavailable register for %s: value=0x%X matches error value 0x%X", 
                             data_type, int_value, error_value)
                return True
                ...
                if len(raw_registers) >= 2:
                    ...
                    if data_type.lower() == "uint32":
                        ...
                        if high_word == 0xFFFF and low_word == 0xFFFF:
                            _LOGGER.debug("Detected unavailable uint32 register: both words are 0xFFFF")
                            return True
                    elif data_type.lower() == "int32":
                        # For int32, both words should be 0x7FFF
                        if high_word == 0x7FFF and low_word == 0x7FFF:
                            _LOGGER.debug("Detected unavailable int32 register: both words are 0x7FFF")
                            return True
...
def handle_unavailable_register(data_type: str, register_name: str) -> Any:
    ...
    try:
        ...
        if error_value is None:
            _LOGGER.warning("No error value defined for data type %s in register %s", 
                           data_type, register_name)
            return None
        
        _LOGGER.debug("Register %s is unavailable (data_type: %s), returning error value: %s", 
                     register_name, data_type, error_value)
        ...
        _LOGGER.error("Error handling unavailable register %s: %s", register_name, str(e))
        return None
...
def log_register_error(register_name: str, data_type: str, value: Any, error_type: str = "unavailable"):
    ...
    try:
        if error_type == "unavailable":
            _LOGGER.warning("Register %s (%s) is unavailable - value: %s (0x%X)", 
                           register_name, data_type, value, 
                           value if isinstance(value, int) else 0)
        else:
            _LOGGER.warning("Register %s (%s) has %s value: %s", 
                           register_name, data_type, error_type, value)
...
def test_error_handling():
    ...
    for i, (value, data_type, expected, desc) in enumerate(test_cases):
        try:
            ...
            _LOGGER.info("Test %d (%s): %s - value=%s, data_type=%s, expected=%s, got=%s", 
                        i + 1, desc, status, value, data_type, expected, result)
        except Exception as e:
